 ...
 *
 * @example
 * calculateCostBreakdown('claude', 'opus-4.6', { inputTokens: 1000, outputTokens: 500 })
 * // Returns: {
 * //   totalCost: 0.0525,
 * //   inputCost: 0.015,
 * //   outputCost: 0.0375,
 ...
 * // }
 */
export function calculateCostBreakdown(provider, modelId, usage) {
  if (!provider || !modelId) {
    return {
      totalCost: 0,
      inputCost: 0,
 ...
 * @param {string} requests[].provider - Provider name
 * @param {string} requests[].modelId - Model ID
 * @param {Object} requests[].usage - Token usage
 * @returns {Object} Batch cost summary
 *
 * @example
 * calculateBatchCost([
 *   { provider: 'claude', modelId: 'opus-4.6', usage: { inputTokens: 1000, outputTokens: 500 } },
 *   { provider: 'gemini', modelId: 'gemini-3-pro', usage: { inputTokens: 2000, outputTokens: 1000 } }
 * ])
 * // Returns: {
 * //   totalCost: 0.115,
 * //   requests: 2,
 ...
 * // }
 */
export function calculateBatchCost(requests) {
  ...
  const breakdown = requests.map((req, index) => {
    const cost = calculateCost(req.provider, req.modelId, req.usage);
    return {
      index,
      provider: req.provider,
      modelId: req.modelId,
      cost,
      usage: validateUsage(req.usage)
    };
  });
