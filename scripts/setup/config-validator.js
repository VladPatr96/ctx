...
export function validateProvider(provider) {
  ...
  // If provider is not available, return early
  if (!provider.available) {
    return {
      status: 'invalid',
  ...
  switch (provider.id) {
    case 'claude':
      return validateClaude(provider);
    case 'codex':
      return validateCodex(provider);
    case 'gemini':
      return validateGemini(provider);
    case 'opencode':
      return validateOpenCode(provider);
    default:
      return {
        status: 'invalid',
        message: `Unknown provider type: ${provider.id}`
      };
  }
}
 ...
 * @param {Array<Object>} providers - Array of provider objects from detector
 * @returns {Array<{provider: Object, validation: ValidationResult}>} Validation results
 */
